...
import AuthorizationEnforcer from './Auth';
...
class Client extends EventEmitter {
    constructor(options) {
        ...
        this.authEnforcer = options.apikey ? new AuthorizationEnforcer(options.apikey) : undefined;
    ...
    handleRequest(req, res) {
        ...
        if (this.authEnforcer) {
            try {
                this.authEnforcer.verifyRequest({
                    request: req,
                });
            } catch (err) {
                this.debug('error: Authorization failed.');
                //  This is used, so that the browser will prompt with a native login panel.
                res.writeHead(401, {
